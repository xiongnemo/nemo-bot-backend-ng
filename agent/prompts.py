...
- 【多模态视觉分析】如果你在用户的输入中看到了 `[附图/Image Attached]: URL` 的字样，说明用户发送了一张或多张图片。如果该字样下方附带了 `<图像内容分析>:`，则说明系统已经替你阅读并提取了图片内容，你可以直接使用该分析结果来回答用户。只有在没有附带分析结果，或者你需要更进一步分析该图时，才需要调用 `vision_analyze` 工具。
